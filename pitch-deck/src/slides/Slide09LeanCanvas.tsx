...
function Cell({ title, color, children, className = '' }: {
  ...
  return (
    <motion.div variants={fadeUp} className={`bg-card rounded-lg p-4 ${className}`}>
...
export function Slide09LeanCanvas() {
  return (
    ...
    >
      ...
      {/* Row 1: 5 columns */}
      <div className="grid grid-cols-[1fr_1fr_1fr_0.8fr_1fr] gap-3 mt-4 flex-1">
        <Cell title="問題" color="text-terra">
          ...
          <div className="mt-3 pt-2 border-t border-cream-dim/20">
            <p className="text-sm font-bold text-cream-dim mb-1">現有替代方案</p>
        ...
        <Cell title="獨特價值主張" color="text-gold">
          <p className="text-cream font-bold text-sm">讓報價階段的錯誤，不留到施工階段</p>
          <Li dim>不是報價工具，是風險管理系統</Li>
          <Li dim>培養設計師的底氣，不是取代設計師</Li>
          <div className="mt-3 pt-2 border-t border-cream-dim/20">
            <p className="text-sm font-bold text-gold mb-1">不公平競爭優勢</p>
            <Li>賀云本人就是目標用戶</Li>
        ...
        <Cell title="目標客群" color="text-sage">
          <p className="text-gold font-bold text-sm">使用者：設計師（先免費用起來）</p>
          <p className="text-gold font-bold text-sm">付費者：進階功能的設計師或公司</p>
          <Li dim>3 人以上的中型設計公司</Li>
          <Li dim>年輕設計師需要資深覆核的團隊</Li>
          <div className="mt-3 pt-2 border-t border-cream-dim/20">
            <p className="text-sm font-bold text-cream-dim mb-1">通路</p>
      ...
      <div className="grid grid-cols-2 gap-3 mt-3 mb-2">
        ...
        <Cell title="收入來源" color="text-sage">
          ...
          <p className="text-gold font-bold text-sm mt-1">一次漏項損失 10-30 萬，月費 NT$ 990 幾乎零風險</p>
        </Cell>
      </div>

      <motion.p variants={fadeUp} className="text-sm text-clay font-bold text-center pb-2">
